...
def constructBayesNet(gameState):
    ...
    for level1 in [X_POS_VAR, Y_POS_VAR]:
        for level2 in [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR]:
            edges.append((level1, level2))
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    result = inference.inferenceByVariableElimination(bayesNet, HOUSE_VARS, evidence, eliminationOrder)
    dicts = result.getAllPossibleAssignmentDicts()
    probs = [result.getProbability(a) for a in dicts]
    highest_prob = dicts[probs.index(max(probs))]

    return {'foodHouse': highest_prob['foodHouse']}
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        win = WON_GAME_REWARD
        loss = GHOST_COLLISION_REWARD
        result = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)
        dicts = result.getAllPossibleAssignmentDicts()
        tl_food = {'foodHouse': 'topLeft', 'ghostHouse': 'topRight'}
        tr_food = {'foodHouse': 'topRight', 'ghostHouse': 'topLeft'}
        prob_tl, prob_tr = 0, 0
        for d in dicts:
            if tl_food.items() <= d.items():
                prob_tl = result.getProbability(d)
            if tr_food.items() <= d.items():
                prob_tr = result.getProbability(d)

        leftExpectedValue = (prob_tl * win) + ((1 - prob_tl) * loss)
        rightExpectedValue = (prob_tr * win) + ((1 - prob_tr) * loss)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        expectedValue = 0
        exploration_outcomes = [a[1] for a in self.getExplorationProbsAndOutcomes(evidence)]
        exploration_probs = [a[0] for a in self.getExplorationProbsAndOutcomes(evidence)]
        for outcome, prob in zip(exploration_outcomes, exploration_probs):
            enter_vals = self.computeEnterValues(outcome, enterEliminationOrder)
            max_expected = max(enter_vals[0], enter_vals[1])
            expectedValue += max_expected * prob
