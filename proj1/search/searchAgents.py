...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        space)
        """
        if self.startingPosition in self.cornerset:
            self.cornerset.remove(self.startingPosition)
        return self.startingPosition, self.cornerset
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            cost = self.getActionCost(state, action, next)
            if cost != 999999:
                children.append((next, action, cost))
        self._expanded += 1 # DO NOT CHANGE
        return children
    ...
    def getNextState(self, state, action):
        ...
        dx, dy = Actions.directionToVector(action)
        nextx, nexty = int(x + dx), int(y + dy)
        if (nextx, nexty) in state[1].copy():
            nextstate = state[1].copy()
            nextstate.remove((nextx, nexty))
            return (nextx, nexty), nextstate
...
def cornersHeuristic(state, problem):
    ...
    if len(state[1]) == 0:
        ...
        manhattan = [(abs(state[0][0] - corner[0]) + abs(state[0][1] - corner[1])) for corner in state[1]]
        # euclidean = [(((state[0][0] - corner[0]) ** 2 + (state[0][1] - corner[1]) ** 2 ) ** 0.5) for corner in state[1]]
...
def foodHeuristic(state, problem):
    ...
    if len(foodGrid.asList()) == 0:
        ...
        distances = [mazeDistance(state[0], food, problem.startingGameState) for food in foodGrid.asList()]
        # euclidean = [(((state[0][0] - food[0]) ** 2 + (state[0][1] - food[1]) ** 2 ) ** 0.5) for food in foodlist]
